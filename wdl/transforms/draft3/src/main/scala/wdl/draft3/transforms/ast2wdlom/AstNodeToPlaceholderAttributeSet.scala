...
import wdl.draft3.parser.WdlParser.{Ast, AstList}
...
object AstNodeToPlaceholderAttributeSet {
  ...
  private def validAttributeSet(a: Vector[PlaceholderAttributeElement]): Checked[PlaceholderAttributeSet] = {
    def foldFunction(acc: (PlaceholderAttributeSet, List[String]), next: PlaceholderAttributeElement): (PlaceholderAttributeSet, List[String]) = {
      (acc, next) match {
        case ((PlaceholderAttributeSet(None, t, f, s), errors), DefaultAttributeElement(d)) => (PlaceholderAttributeSet(Option(d), t, f, s), errors)
        case ((pas @ PlaceholderAttributeSet(Some(d1), _, _, _), errors), DefaultAttributeElement(d2)) => (pas, errors :+ s"""Cannot supply 'default="$d2"' because it is already supplied as 'default="$d1"'""")

        case ((PlaceholderAttributeSet(d, None, f, s), errors), TrueAttributeElement(t)) => (PlaceholderAttributeSet(d, Option(t), f, s), errors)
        case ((pas @ PlaceholderAttributeSet(_, Some(t1), _, _), errors), TrueAttributeElement(t2)) => (pas, errors :+ s"""Cannot supply 'true="$t2"' because it is already supplied as 'true="$t1"'""")

        case ((PlaceholderAttributeSet(d, t, None, s), errors), FalseAttributeElement(f)) => (PlaceholderAttributeSet(d, t, Option(f), s), errors)
        case ((pas @ PlaceholderAttributeSet(_, _, Some(f1), _), errors), FalseAttributeElement(f2)) => (pas, errors :+ s"""Cannot supply 'false="$f2"' because it is already supplied as 'false="$f1"'""")

        case ((PlaceholderAttributeSet(d, t, f, None), errors), SepAttributeElement(s)) => (PlaceholderAttributeSet(d, t, f, Option(s)), errors)
        case ((pas @ PlaceholderAttributeSet(_, _, _, Some(s1)), errors), SepAttributeElement(s2)) => (pas, errors :+ s"""Cannot supply 'sep="$s2"' because it is already supplied as 'sep="$s1"'""")
      }
    }
  ...
  private def convertAttributeKvp(a: Ast): ErrorOr[PlaceholderAttributeElement] = {
  ...
  private def placeholderAttributeConstructor(kvpAst: Ast): ErrorOr[String => PlaceholderAttributeElement] = {
  ...
  private def placeholderAttributeValue(kvpAst: Ast): ErrorOr[String] = {
