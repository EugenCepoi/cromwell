...
object AstNodeToMetaValueElement {
  ...
  def astNodeToMetaValueElement(implicit astNodeToMetaKvPair: CheckedAtoB[GenericAstNode, MetaKvPair]
                               ): CheckedAtoB[GenericAstNode, MetaValueElement] = CheckedAtoB.fromErrorOr("convert AstNode to MetaValueElement") { astNode =>

    lazy implicit val thisConversion = astNodeToMetaValueElement(astNodeToMetaKvPair)
